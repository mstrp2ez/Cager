var EntityMngr=function(){
	...
	this.m_EntityConversionTable=["Player","Torch","Projectile","Mine","HealthPack"];
	...
	this.Update=function(p_Delta){
		...
		if(xSF.length>0){
			...
			var timestamp=xSF[0].detail.t;
			var timeDiff=Date.now()-timestamp;
			if(timeDiff<xThis.m_TimeDelay){return;}
			xSF.splice(0,1);
			var offset=0;
	...
	this.SaveState=function(){
		...
		for (var obj in xThis.m_Entities){
			var o=xThis.m_Entities[obj];
			if(o&&o.SaveState===undefined){continue;}
			o.SaveState();
		}
	}
	document.addEventListener('NetSnapshot', function(p_Data){
		var xSF=xThis.m_xSnapshotFrames;
		xSF.push(p_Data);
		xThis.m_bFullSnapReceived=true;
	});
		//if(xSF.length>=3||!xThis.m_bFullSnapReceived){
			
	//	}
	//first snapshot should always be a full one
